...
NemuMMC::NemuMMC(const NemuMMCParams *p) : BasicPioDevice(*p,p->pio_size),
    blkcnt(0),blk_addr(0),tmp_addr(0),write_cmd(false),read_ext_csd(false)
{
    assert(C_SIZE < (1 << 12));
...
inline void NemuMMC::prepare_rw(int is_write) {
  blk_addr = sd_reg_base[SDARG];
  tmp_addr = 0;
  if (img_fp) fseek(img_fp, blk_addr << 9, SEEK_SET);
  write_cmd = is_write;
}

void NemuMMC::sdcard_handle_cmd(int cmd) {
  switch (cmd) {
    case MMC_GO_IDLE_STATE: break;
    case MMC_SEND_OP_COND: sd_reg_base[SDRSP0] = 0x80ff8000; break;
    ...
    case MMC_SEND_CSD:
      ...
      sd_reg_base[SDRSP2] = 0x0f508000 | (C_SIZE >> 2) | (READ_BL_LEN << 16);
      sd_reg_base[SDRSP3] = 0x9026012a;
      break;
    case MMC_SEND_EXT_CSD: read_ext_csd = true; tmp_addr = 0; break;
    case MMC_SLEEP_AWAKE: break;
    case MMC_APP_CMD: break;
    case MMC_SET_RELATIVE_ADDR: break;
    case MMC_SELECT_CARD: break;
    case MMC_SET_BLOCK_COUNT: blkcnt = sd_reg_base[SDARG] & 0xffff; break;
...
void NemuMMC::sdcard_io_handler(uint32_t offset) {
  assert(img_fp);
  assert(sdfp);
  int idx = offset / 4;
  switch (idx) {
    case SDCMD: sdcard_handle_cmd(sd_reg_base[SDCMD] & 0x3f); break;
    case SDARG:
    case SDRSP0:
    case SDRSP1:
    case SDRSP2:
    ...
    case SDDATA:
       if (read_ext_csd) {
         ...
         if (tmp_addr == 512 - 4) read_ext_csd = false;
       } else if (img_fp) {
         __attribute__((unused)) int ret;
         if (!write_cmd) { ret = fread(&sd_reg_base[SDDATA], 4, 1, img_fp); }
         else { ret = fwrite(&sd_reg_base[SDDATA], 4, 1, img_fp); }
       }
       tmp_addr += 4;
       break;
    default:
...
void NemuMMC::unserialize_sdcard(FILE *sdfp) {
  __attribute__((unused)) int ret;
...
Tick NemuMMC::read(PacketPtr pkt) {
...
Tick NemuMMC::write(PacketPtr pkt) {
    assert(pkt->getSize() == 4);
    Addr offset = pkt->getAddr() - pioAddr;
