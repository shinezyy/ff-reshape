...
outdir =  f'{c.stats_base_dir_06}/xbar4-rand-hint{d}'
...
def op_rand(benchmark, some_extra_args, outdir_b, cpt_id):
    ...
    options = [
            ...
            '--checkpoint-dir={}'.format(pjoin(c.gem5_cpt_dir(arch, 2006),
                benchmark)),
            '--arch={}'.format(arch),
            '--spec-size=ref',
            ]
...
def run(benchmark_cpt_id):
    ...
    cpt_flag_file = pjoin(c.gem5_cpt_dir(arch, 2006), benchmark,
    ...
    with open('./all_function_spec.txt') as f:
        for line in f:
            if not line.startswith('#'):
                benchmarks.append((line.strip(), 0))
